...
/* ----------------------------------------------------------------------
   ...
   Configurational Bias Grand Canonical Monte Carlo: William Watkins
------------------------------------------------------------------------- */

#include "fix_cbgcmc.h"
...
FixCBGCMC::FixCBGCMC(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg),
...
{
  if (narg < 11) error->all(FLERR,"Illegal fix cbgcmc command");        

  if (atom->molecular == Atom::TEMPLATE)
    error->all(FLERR,"Fix cbgcmc does not (yet) work with atom_style template");
  ...
  if (regionflag) {
    if (domain->regions[iregion]->bboxflag == 0)
      error->all(FLERR,"Fix cbgcmc region does not support a bounding box");
    if (domain->regions[iregion]->dynamic_check())
      error->all(FLERR,"Fix cbgcmc region cannot be dynamic");
...
void FixCBGCMC::options(int narg, char **arg)
{
  ...
  min_ngas = -1;
  max_ngas = INT_MAX;
  // my extra parameter to check for maximum cutoff
  // for full_energy I would not like to recalculate the energy
  // if the particle is far away from its neighbors to not effect the energy
  max_cutoffsq = 0.0;
  max_cutoff_flag = 0;
  ...
  if (strcmp(arg[iarg],"mol") == 0) {
      ...
      iarg += 2;
    } else if (strcmp(arg[iarg],"max_cutoff") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix cbgcmc command");
      double rtmp = utils::numeric(FLERR,arg[iarg+1],false,lmp);
      max_cutoffsq = rtmp*rtmp;
      max_cutoff_flag = 1;
      iarg += 2;
...
void FixCBGCMC::attempt_atomic_translation()
{
...
void FixCBGCMC::attempt_atomic_deletion()
{
...
void FixCBGCMC::attempt_atomic_insertion()
{
  double lamda[3];
...
void FixCBGCMC::attempt_molecule_translation()
{
  ntranslation_attempts += 1.0;
...
void FixCBGCMC::attempt_molecule_rotation()
{
...
void FixCBGCMC::attempt_molecule_deletion()
{
  ndeletion_attempts += 1.0;
...
void FixCBGCMC::attempt_molecule_insertion()
{
  double lamda[3];
  ninsertion_attempts += 1.0;
...
void FixCBGCMC::attempt_atomic_translation_full()
{
  ntranslation_attempts += 1.0;
...
void FixCBGCMC::attempt_atomic_deletion_full()
{
  double q_tmp;
...
void FixCBGCMC::attempt_atomic_insertion_full()
{
  double lamda[3];
  ninsertion_attempts += 1.0;
...
void FixCBGCMC::attempt_molecule_translation_full()
{
  ntranslation_attempts += 1.0;
...
void FixCBGCMC::attempt_molecule_rotation_full()
{
  nrotation_attempts += 1.0;
...
void FixCBGCMC::attempt_molecule_deletion_full()
{
  ndeletion_attempts += 1.0;
...
void FixCBGCMC::attempt_molecule_insertion_full()
{
  double lamda[3];
  ninsertion_attempts += 1.0;
...
double FixCBGCMC::energy(int i, int itype, tagint imolecule, double *coord)
{
...
double FixCBGCMC::molecule_energy(tagint gas_molecule_id)
{
...
/* ----------------------------------------------------------------------
   check if distance is good, i.e. atom is not too close or far away
------------------------------------------------------------------------- */
int FixCBGCMC::check_cutoffs(double *coord)
{
  if( !( overlap_flag || max_cutoff_flag ) ) return 1;
  int overlaptestall;
  int overlaptest = 0;
  int max_cutofftestall;
  int max_cutofftest = 1;
    
  double delx,dely,delz,rsq;
  double **x = atom->x;
  tagint *molecule = atom->molecule;
  int nall = atom->nlocal + atom->nghost;
  // check max cutoff after checking if atoms are too close
  // only calculate a max cutoff if calculating energy for after the atom is added
  for (int j = 0; j < nall; j++) {
    delx = coord[0] - x[j][0];
    dely = coord[1] - x[j][1];
    delz = coord[2] - x[j][2];
    ...
    if (rsq < max_cutoffsq) {
      max_cutofftest = 0; 
      if ( !overlap_flag ) return 1;
    }
    if (rsq < overlap_cutoffsq) {
      overlaptest = 1;
      if ( overlap_flag ) return 0;
    }
  }
  //MPI_Allreduce(&max_cutofftest, &max_cutofftestall, 1,
  //            MPI_INT, MPI_MAX, world);
  // if true don't recalculate energy, the particle is considered 'far enough' to not affect total energy
  if (max_cutofftest && max_cutoff_flag) return 0;
...
int FixCBGCMC::pick_random_gas_atom()
{
  int i = -1;
...
tagint FixCBGCMC::pick_random_gas_molecule()
{
  int iwhichglobal = static_cast<int> (ngas*random_equal->uniform());
...
void FixCBGCMC::toggle_intramolecular(int i)
{
...
void FixCBGCMC::update_gas_atoms_list()
{
...
double FixCBGCMC::compute_vector(int n)
{
...
void FixCBGCMC::write_restart(FILE *fp)
{
  int n = 0;
  double list[12];
...
void FixCBGCMC::restart(char *buf)
{
  int n = 0;
...
void FixCBGCMC::grow_molecule_arrays(int nmolatoms) {
