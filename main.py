...
def main():
    parser = argparse.ArgumentParser(
                description='Run an ab initio Grand Canonical Monte Carlo simulation using QE and LAMMPS')
    parser.add_argument("-v", "--verbose", help="increase output verbosity",
                    action="store_true")
    parser.add_argument("-i", "--in", "--input", dest="xsf", default="structure.xsf",
                    help="input structure as XSF")
    parser.add_argument("-e", "--el", dest="el", default="el_list.txt",
                    help="file with list of elements and associated constants")
    parser.add_argument("-l", "--lmp", dest="lmp_init", default="in.gcmc",
                    help="LAMMPS input scripts")
    parser.add_argument("-d", "--dump", dest="dump", default="gcmc.tmp",
                                help="LAMMPS dump file, i.e. structure snapshots")

    parser.add_argument('-T', '--temp', type=int, default=500, help='temperature for GCMC algo (default: 500)')
    ...
    # READ INPUT FILES #
    ####################
    xsf_filename   = params['xsf'] # read xsf filename
    el_filename    = params['el'] # read element list filename

    lmp_init       = params['lmp_init']
    dump_file      = params['dump']
    step_max       = 100
    ...
    biasProposals = True       # don't use lammps, just the basic random structure proposal
    ...
    ###################################
    if biasProposals:
        mc_run = MonteCarloLammps(T_move, max_disp, xsf)
    else:
    ...
    for i in range(niter) :
        ...
        # this is commented out because mpiexec_mpt is for PBS systems
        #call_qe = 'mpiexec_mpt -np ' + str(nproc) + ' ../bin/pw.x -nk ' + str(nkdiv) + ' -ndiag ' + str(ndiag) + ' -i qe.in > qe.out'
        call_qe = '../bin/pw.x -nk ' + str(nkdiv) + ' -ndiag ' + str(ndiag) + ' -i qe.in > qe.out'
